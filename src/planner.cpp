...
Planner::Planner() {
  ...
  // TOPICS TO SUBSCRIBE
  if (Constants::manual) {
    std::cout << "sub /map" << std::endl;
    subMap = n.subscribe("/map", 1, &Planner::setMap, this);
  } else {
    std::cout << "sub /occ_map" << std::endl;
...
void Planner::setMap(const nav_msgs::OccupancyGrid::Ptr map) {
  ...
  std::string path = ros::package::getPath("hybrid_astar");
  auto file = path + "/launch/result.ppm";
  voronoiDiagram.initializeMap(width, height, binMap);
  voronoiDiagram.update();
  voronoiDiagram.visualize(file.c_str());
